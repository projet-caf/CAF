...
class ValuesSubscriber implements EventSubscriberInterface
{
    ...
    public function preSetData(DataEvent $event)
    {
        ...
        foreach($this->fields as $field) {
            ...
            $field_taxonomy = strtolower($field->getIdFieldTaxonomy()->getName());
            if($field_taxonomy == 'date') {
                $form->add($this->factory->createNamed($field->getName(),'text', null, array('required' => $field->getRequired(), 'label' => $field->getLibelle(), 'attr' => array('class' => 'datepicker')))); 
